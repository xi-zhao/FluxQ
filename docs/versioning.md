...
- `0.3.x`
  ...
  `0.3.1` adds agent-observability surfaces: `health`, `reason_codes`, `next_actions`, decision or gate blocks, and `--jsonl` event streams for long-running commands.
  Next `0.3.x` work should deepen prompt ingress and compare policy detail without changing the core runtime truth layers.
